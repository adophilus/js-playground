...
import { type User, CurrentUser, UserId } from './User'
...
export class Unauthorized extends Schema.TaggedError<Unauthorized>()(
  ...
  static refail(entity: string, action: string) {
    ...
    ): Effect.Effect<AbortController, Unauthorized, CurrentUser | R> =>
      Effect.catchIf(
        effect,
        (e) => !Unauthorized.is(e),
        () =>
...
export const withSystemActor = <A, E, R>(
  effect: Effect.Effect<A, E, R>
): Effect.Effect<A, E, Exclude<R, AuthorizedActor<any, any>>> => effect
